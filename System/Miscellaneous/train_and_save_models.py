    ...
    try:
        ...
        if X_test is not None and y_test is not None:
            ...
            ocknn_precision, ocknn_recall, ocknn_f1, ocknn_far, ocknn_frr, ocknn_cm, ocknn_eer = evaluate_model(ocknn_model, X_test, y_test)
             ...
             # Print metrics
            print(f"SVDD Metrics for {user_id}:")
            print(f"Precision: {svdd_precision}")
            print(f"Recall: {svdd_recall}")
            print(f"F1 Score: {svdd_f1}")
            print(f"FAR: {svdd_far}")
            print(f"FRR: {svdd_frr}")
            print(f"EER: {svdd_eer}")
            sys.stdout.flush() 
            
            print(f"OCKNN Metrics for {user_id}:")
            ...
            print(f"EER: {ocknn_eer}")
            sys.stdout.flush() 

            # Plot and save confusion matrices
            print(f"SVDD Confusion Matrix for {user_id}:")
            print(svdd_cm)
            sys.stdout.flush()  # Ensure prints are flushed
            plot_confusion_matrix(svdd_cm, 'SVDD')
            
            print(f"OCKNN Confusion Matrix for {user_id}:")
            print(ocknn_cm)
            sys.stdout.flush()  # Ensure prints are flushed
